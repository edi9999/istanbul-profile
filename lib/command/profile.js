...
ProfileCommand.TYPE = 'profile';
util.inherits(ProfileCommand, Command);

Command.mix(ProfileCommand, {
   synopsis: function synopsis() {
      return "instruments for profiling a file or a directory tree and writes the instrumented code to the desired output location";
   },
   ...
   run: function(args, callback) {
      ...
      instrumenter = new Instrumenter({
         ...
         preambleArray: ['var profile=(profile?profile:(function(){var l=9;var path="./profile";while (l--){try{return require(path);}catch(err){path="../"+path;}}})());'],
   ...
   coverFunctionProfiler: function(node, walker) {
      try {
      if (node.body.body[0].leadingComments[0].value === "profile-ignore")
         return;
      }
      catch (err) {
      }
      ...
      if (!node.id) {
         var newName = null;
         var parent = walker.path[walker.path.length - 1];
         if (parent.node.type === "AssignmentExpression") {
            if (parent.node.left.type === "MemberExpression") {
               var newName = "";
               if (parent.node.left.object.name)
                  newName += parent.node.left.object.name + ".";
               newName += parent.node.left.property.name || parent.node.left.property.value;
               fnName = newName;
            }
         } else {
            if (parent.node.type === "Property") {
               newName = parent.node.key.name;
               fnName = newName;
            }
         }
      }
      ...
      if (fnName) {
         ...
         if (node.params.length > 0)
            lastParamName = node.params[node.params.length - 1].name;
         ...
         if (lastParamName && lastParamName === "cb") {
            // we have to limit the name, some cases, you are just passing in the last parameters a function, but not cb...
